import { Component, Input, Directive, Optional, Self } from "@angular/core";
...
@Directive({
  selector: '[HideSelection]',
  standalone: true
})
export class HideSelectionDirective {}
  ...
  <ScrollViewer>
    <StackPanel Padding="2px 0" [ngStyle]="{
      ...
      <ItemContainer #container *ngFor="let item of ItemSource; index as index; trackBy: getValue" class="item" (click)="onItemClick($event, index, item)" [id]="'xaml-selector-'+_id+'-item-'+index" @itemFading>
        ...
        <div class="selection-indicator" *ngIf="!hideSelection && index === SelectedIndex"></div>
      </ItemContainer>
    <StackPanel/>
  <ScrollViewer/>`,
  animations: [
...
export class GridView extends SelectorComponent {
  @Input({ required: true }) ColumnCount: number = 1;
  constructor(@Optional() @Self() public hideSelection?: HideSelectionDirective) {
    super();
  }
}
